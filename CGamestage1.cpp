...
void CGamestage1::OnInit() {
	...
	for (int i = 0; i < 8; i++) {
		bullet_of_monster[i].push_back(CMovingBitmap());
		bullet_of_monster[i][0].LoadBitmapByString({ "Resources/weapon/bullet.bmp" }, RGB(255, 255, 255));
		random_born_bullet_monster(bullet_monster, {
			"Resources/monster/bullet_monster/purple/f0.bmp" , "Resources/monster/bullet_monster/purple/f1.bmp", "Resources/monster/bullet_monster/purple/f2.bmp", "Resources/monster/bullet_monster/purple/f3.bmp",
			"Resources/monster/bullet_monster/purple/f4.bmp" , "Resources/monster/bullet_monster/purple/f5.bmp", "Resources/monster/bullet_monster/purple/f6.bmp", "Resources/monster/bullet_monster/purple/f7.bmp",
			"Resources/monster/bullet_monster/purple/f8.bmp", "Resources/monster/bullet_monster/purple/f9.bmp", "Resources/monster/bullet_monster/purple/f10.bmp", "Resources/monster/bullet_monster/purple/f11.bmp",
			"Resources/monster/bullet_monster/purple/f12.bmp", "Resources/monster/bullet_monster/purple/f13.bmp", "Resources/monster/bullet_monster/purple/f14.bmp", "Resources/monster/bullet_monster/purple/f15.bmp" },
			monster_vanish, { "Resources/monster/m11.bmp", "Resources/monster/m12.bmp", "Resources/monster/m13.bmp", "Resources/monster/m14.bmp", "Resources/monster/m15.bmp",
			"Resources/monster/m16.bmp", "Resources/monster/m17.bmp" }, { 255, 255, 255 }, { 200, 191, 231 }, i);  //255
		bullet_monster[i].SetAnimation(50, false);
		bullet_monster[i].set_hp(6);
		bullet_monster[i].set_end = 30;
	}
	...
	int magnet_position[2] = { -300, 300 };
	for (int i = 0; i < 2; i++) {
		magnet.push_back(CMovingBitmap());
		magnet[i].LoadBitmapByString({ "Resources/props/magnet.bmp" , "Resources/ignore.bmp" }, RGB(255, 255, 255));
		magnet[i].SetTopLeft(character.GetLeft() + magnet_position[i], character.GetTop() - 300);
		magnet_timer.push_back(int());
		magnet_trigger.push_back(int());
		magnet_once.push_back(int());
		magnet_timer[i] = 0;
		magnet_trigger[i] = 0;
		magnet_once[i] = 0;
	}
...
void CGamestage1::OnMove() {
	...
	for (int i = 0; i < (int)(magnet.size()); i++) {
		item_move(magnet[i]);
		if (character.IsOverlap(character, magnet[i])) {
			magnet_trigger[i] = 1;
			magnet[i].SetFrameIndexOfBitmap(1);
		}
		if (magnet_trigger[i] == 1 && magnet_once[i] == 0) {
			magnet_timer[i] += 1;

			if (magnet_timer[i] % 2 == 0) {
				magnet_animation();
			}
			if (magnet_timer[i] > 100) {
				magnet_trigger[i] = 0;
				magnet_timer[i] = 0;
				magnet_once[i] = 1;
			}
...
void CGamestage1::show_img() {
	...
	for (int i = 0; i < (int)(bullet_monster.size()); i++) {
		/*if (bullet_monster[i].GetFrameIndexOfBitmap() >= bullet_monster[i].limit_frame_end) {
			bullet_monster[i].SetFrameIndexOfBitmap(bullet_monster[i].limit_frame_start);
		}*/
		if (bullet_monster[i].IsOverlap(background, bullet_monster[i])) {
			bullet_monster[i].ShowBitmap();
		}
	}

	for (int i = 0; i < (int)(bullet_monster_vanish.size()); i++) {
		bullet_monster_vanish[i].ShowBitmap();
	}
	...
	for (int i = 0; i < 8; i++) {
		//for (int j = 0; j < (int)bullet_of_monster[i].size(); j++) {
			bullet_of_monster[i][0].ShowBitmap();
		//}
	}
...
void CGamestage1::random_born_bullet_monster(vector<CMovingBitmap>&monster, vector<string> str_monster, vector<CMovingBitmap>&monster_vanish, vector<string> str_monster_vanish, vector<int>rgb_monster, vector<int>rgb_monster_vanish, int i) {
	...
	if (i % 4 == 0) {
		...
		monster[tail].SetTopLeft(monster[4 * (i / 4)].GetLeft() + 70 * (i % 4), monster[4 * (i / 4)].GetTop());
		monster[tail].set_center(monster[4 * (i / 4)].GetLeft() + 70 * (i % 4) + 45, monster[4 * (i / 4)].GetLeft() + 57);
	}

	bullet_of_monster[i][0].SetTopLeft(monster[tail].GetLeft() + 10, monster[tail].GetTop() - 10);
	...
	/*if (isLeft(character, monster[tail])) {
		...
		monster[tail].set_limit_start_end(4, 7);
		monster[tail].SetFrameIndexOfBitmap(monster[tail].limit_frame_start);
...
void CGamestage1::random_born_big_monster(vector<CMovingBitmap>&monster, vector<string> str_monster, vector<CMovingBitmap>&monster_vanish, vector<string> str_monster_vanish, vector<int>rgb_monster, vector<int>rgb_monster_vanish) {
...
void CGamestage1::monster_all() {
	...
	for (int i = 0; i < (int)(big_monster.size()); i++) {
		...
		if (!big_monster[i].IsOverlap(character, big_monster[i])) {
	...
	for (int i = 0; i < (int)(bullet_monster.size()); i++) {
		item_move(bullet_monster[i]);
		if (!bullet_monster[i].IsOverlap(character, bullet_monster[i])) {
			monster_move(bullet_monster[i]);
			blood.SetAnimation(50, true);
			//if (abs(bullet_monster[i].GetLeft() - character.GetLeft()) < 1065 / 2) {
			//if (bullet_monster[i].GetTop() > 0 || bullet_monster[i].GetTop() + 30 < 1065 || bullet_monster[i].GetLeft() > 0 || bullet_monster[i].GetLeft() + bullet_monster[i].GetWidth() < 1065) { //in range
				if (bullet_of_monster_track[i][0] == 0 && bullet_of_monster_track[i][1] == 0 && bullet_of_monster_track[i][2] == 0) {
					bullet_of_monster_generate_track(bullet_of_monster[i], bullet_monster[i], i);
				}
			//}
			if (bullet_of_monster_track[i][0] != 0 && bullet_of_monster_track[i][1] != 0) { //any range
				int m = bullet_of_monster_track[i][0];
				int delta_x = bullet_of_monster_track[i][1];
				int b = bullet_of_monster_track[i][2];
				int x = bullet_of_monster[i][0].GetLeft() + delta_x;
				int y = m * x + b;
				bullet_of_monster[i][0].SetTopLeft(x, y);
				bullet_of_monster_erase(bullet_of_monster[i], bullet_monster[i], i);
			}
		}
		else if(bullet_monster[i].IsOverlap(character, bullet_monster[i])) {
			character.add_sub_hp(-5);
			blood.SetAnimation(50, false);
			blood.ShowBitmap();
		}
	...
	if (int(bullet_monster.size()) < 10 && bullet_monster_vanish.size() != 0) {
		bullet_monster.push_back(bullet_monster_vanish[0]);
		bullet_monster[bullet_monster.size() - 1].SetFrameIndexOfBitmap(0);
		bullet_monster[bullet_monster.size() - 1].SetAnimation(50, false);
		bullet_monster[bullet_monster.size() - 1].set_hp(6);
		monster_reset(bullet_monster[bullet_monster.size() - 1]);
		bullet_monster_vanish.erase(bullet_monster_vanish.begin());
	}
...
void CGamestage1::big_monster_move(CMovingBitmap &monster) {
...
void CGamestage1::bullet_of_monster_generate_track(vector<CMovingBitmap> &bullet, CMovingBitmap &which_monster, int index) {
	int x1 = character.GetLeft();
	int y1 = character.GetTop();
	for (int i = 0; i < (int)bullet.size(); i++) {
		int x2 = bullet[i].GetLeft();
		int y2 = bullet[i].GetTop();
		int m = 0;
		if (x2 - x1 != 0) {
			m = (y2 - y1) / (x2 - x1);
		}
		...
		bullet_of_monster_track[index][0] = m;
		bullet_of_monster_track[index][1] = x;
		bullet_of_monster_track[index][2] = b;

	}
}


void CGamestage1::bullet_of_monster_erase(vector<CMovingBitmap> &item, CMovingBitmap &which_monster, int index) {
	for (int i = 0; i < (int)item.size(); i++) {
		if (item[i].GetTop() < 0 || item[i].GetTop() + 30 > 1065 || item[i].GetLeft() < 0 || item[i].GetLeft() + item[i].GetWidth() > 1065) {
		//if(abs(bullet_monster[i].GetLeft() - character.GetLeft()) > 1065 / 2){
			item[i].SetTopLeft(which_monster.GetLeft() + 10, which_monster.GetTop() - 10);
			bullet_of_monster_track[index][0] = 0;
			bullet_of_monster_track[index][1] = 0;
			bullet_of_monster_track[index][2] = 0;
		}
	}
}

void CGamestage1::born_bullet_of_monster(vector<CMovingBitmap> &item, CMovingBitmap &which_monster, vector<string> str, vector<int>rgb) {
	int tail = item.size();
	if (item[tail - 1].GetTop() < 0) {
		item.push_back(CMovingBitmap());
		item[tail].LoadBitmapByString(str, RGB(rgb[0], rgb[1], rgb[2]));
		item[tail].SetTopLeft(which_monster.GetLeft() + 10, which_monster.GetTop() - 10);
	}
}

void CGamestage1::bricks_move(CMovingBitmap &item, int h, int k, int c, int x_move) {
...
void CGamestage1::big_monster_born() {
